...
# TODO: check that the symbols cannot contain each other
...
def read_questions(filepath):
    ...
    for line in lines:
        # note: if-statements are structured like this in case the symbols
        # contain each other
        if SUBSUBSECTION_SYMBOL in line:
            ...
            subquestion = Question(
                subquestion_content, None, title, section, subsection, subsubsection
            )
            ...
            if not content is None:
                question = Question(
                    content, subquestions, title, section, subsection, subsubsection
                )
